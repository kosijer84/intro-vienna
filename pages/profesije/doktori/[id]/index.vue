    ...
    <div class="row">
      <div class="col-md-8">
        <img
          ...
          :src="`/professions/doctors/${profession?.headerImg}`"
          class="card-img-top"
          alt="price image"
        >
            ...
            <div class="col-12 mt-4">
              <nav>
                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                  ...
                  <button v-if="professionStaff.length" class="nav-link" id="nav-contact-tab" data-bs-toggle="tab" data-bs-target="#nav-contact" type="button" role="tab" aria-controls="nav-contact" aria-selected="false">
              ...
              <div class="tab-content" id="nav-tabContent">
                <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab" tabindex="0">
                  <div class="mt-4 p-3 text-bg-light bg-gradient row" style="border-radius: 10px">
                    <div class="col">
                      <h6 class="fw-bold">Website:</h6>
                      <p><a :href="profession?.companyExternalLink?.replace(/(^\w+:|^)\/\//, '')" target="_blank">{{profession?.companyExternalLink?.replace(/(^\w+:|^)\/\//, '')}}</a></p>
                      <h6 class="fw-bold">Email:</h6>
                      <p><a :href="profession?.email" target="_blank">{{profession?.workshopPlace?.email}}</a></p>
                      <h6 class="fw-bold">Broj telefona:</h6>
                      <p class="mb-0"><a :href="profession?.number" target="_blank">{{profession?.workshopPlace?.number}}</a></p>
                      <div v-if="profession.insurance">
                        <h6 class="fw-bold">Insurance:</h6>
                        <p>{{profession?.insurance}}</p>
                      </div>
                    </div>
                    <div class="col">
                      <div>
                        <h6 class="fw-bold">Working time:</h6>
                        <p class="mb-0">{{profession?.workshopPlace?.workingTime?.mon}}</p>
                        <p class="mb-0">{{profession?.workshopPlace?.workingTime?.tue}}</p>
                        <p class="mb-0">{{profession?.workshopPlace?.workingTime?.wed}}</p>
                        <p class="mb-0">{{profession?.workshopPlace?.workingTime?.thu}}</p>
                        <p class="mb-0">{{profession?.workshopPlace?.workingTime?.fri}}</p>
                        <p class="mb-0">{{profession?.workshopPlace?.workingTime?.sat}}</p>
                        <p class="mb-0">{{profession?.workshopPlace?.workingTime?.sun}}</p>
                      </div>
                    </div>
                  </div>
...
<script setup lang="ts">
  ...
  function singleProfession() {
    const single = DOCTORS_PROFESSION.filter((e) => e.id === route.params.id)[0]
  ...
  useSeoMeta({
    ...
    ogUrl: () => `https://wien-86d0f.web.app/profesije/doktori/${profession.value?.slug}`
  });
</script>
